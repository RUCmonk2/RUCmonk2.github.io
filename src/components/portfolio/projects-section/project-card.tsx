...
export function ProjectCard({
  ...
  const isStaticResource = href?.startsWith("/assets/") ?? false;
  const MainLink = isStaticResource ? "a" : Link;
    ...
    >
      ...
      <CardHeader className="px-2">
        <div className="space-y-1">
          ...
          <div className="prose text-muted-foreground dark:prose-invert mt-1 mb-1 max-w-full font-sans text-xs text-pretty [&_img]:my-0 [&_img]:inline-block [&_img]:h-[1em] [&_img]:w-auto [&_img]:align-baseline [&_p]:mt-1 [&_p]:mb-1">
            <CustomReactMarkdown>{description}</CustomReactMarkdown>
          </div>
          {authors && authors.trim() !== "" && (
            <div className="prose text-muted-foreground dark:prose-invert mt-1 mb-1 max-w-full font-sans text-xs text-pretty [&_p]:mt-1 [&_p]:mb-1">
